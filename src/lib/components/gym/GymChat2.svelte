<script lang="ts">
  ...
  import { Send, User, Video, Square, Check, X } from 'lucide-svelte';
  ...
  import QuestPopup from './QuestPopup.svelte';
  import QuestPanel from './QuestPanel.svelte';
  import RecordingPanel from './RecordingPanel.svelte';

  const { prompt = '', app = null } = $props<{ prompt?: string; app?: { type: 'executable' | 'website'; name: string; path?: string; url?: string; } | null }>();
  ...
  async function handleQuestData(questData: Record<string, any>) {
    try {
      ...
      if (questData?.title && questData?.app && questData?.icon_url && Array.isArray(questData?.objectives) && questData.objectives.length > 0) {
        currentQuest = {
      ...
      await addMessage(generateAssistantMessage("I encountered an error processing the quest data. Please try again."));
    }
  }
  ...
  async function handleToolCall(toolCall: Record<string, any>) {
    try {
      ...
      await addMessage(generateAssistantMessage("I encountered an error processing the tool call. Please try again."));
    }
  }
  ...
  async function handleGiveUp() {
    if (recordingState === 'recording') {
      ...
      await addMessage({
        role: 'assistant',
        content: "That's okay! Not every task is for everyone. Let me know if you'd like to try something else."
      });
    }
  }
  ...
  async function handleAssistantResponse(response: Record<string, any>) {
    try {
      ...
      await addMessage(generateAssistantMessage("I encountered an error processing the response. Please try again."));
    }
  }
  ...
  {#if showQuestPopup && currentQuest}
    <QuestPopup
      ...
      onDecline={async () => {
        ...
        await addMessage(generateAssistantMessage(
  ...
  <div bind:this={chatContent} class="flex-1 px-6 pb-6 space-y-3 overflow-y-auto chat-content">
    {#each chatMessages as msg, i}
      <div
        ...
        {#if msg.role === 'user'}
          ...
          {:else}
            <Card variant="primary" padding="sm" className="w-auto! max-w-2xl shadow-sm bg-secondary-300 text-white">
        ...
        {:else}
          ...
          {#if msg.content.startsWith('<recording>') && msg.content.endsWith('</recording>')}
            <RecordingPanel recordingId={msg.content.slice(11, -12)} />
          {:else}
            <Card variant="secondary" padding="sm" className="w-auto! shadow-sm space-y-4 relative bg-black/5">
    ...
    {#if isWaitingForResponse}
      <div class="flex gap-2">
        ...
        <Card variant="secondary" padding="sm" className="shadow-sm w-auto! space-y-4 relative bg-black/5">
          <div class="flex gap-0.5 px-1.5 py-1">
            <span class="w-1.5 h-1.5 bg-secondary-300 rounded-full animate-[pulse_1s_ease-in-out_-0.3s_infinite]"></span>
            <span class="w-1.5 h-1.5 bg-secondary-300 rounded-full animate-[pulse_1s_ease-in-out_-0.15s_infinite]"></span>
            <span class="w-1.5 h-1.5 bg-secondary-300 rounded-full animate-[pulse_1s_ease-in-out_0s_infinite]"></span>
          </div>
        </Card>
      </div>
    {/if}
