<script lang="ts">
  ...
  function handleSubmit() {
    const token: Token = selectedTokenType === 'CUSTOM' 
      ? { type: 'CUSTOM', symbol: 'SOL', address: customTokenAddress }
      : { type: selectedTokenType, symbol: selectedTokenType, address: DEFAULT_ADDRESSES[selectedTokenType] };
  ...
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-gray-900 rounded-xl w-full max-w-lg p-6 space-y-4">
      ...
      <form on:submit|preventDefault={handleSubmit} class="space-y-4">
        <div>
          <label class="block text-sm font-semibold text-gray-300 mb-1">Pool Name</label>
          <input
            type="text"
            bind:value={name}
        ...
        <div>
          <label class="block text-sm font-semibold text-gray-300 mb-1">Skills to Collect</label>
          <textarea
            bind:value={skills}
        ...
        <div>
          <label class="block text-sm font-semibold text-gray-300 mb-1">Treasury Token</label>
          <div class="space-y-2">
