<script lang="ts">
  ...
  function updateUnseenOrRunningCount(allJobs: CommandJob[]) {
    ...
    unseenOrRunningCount = allJobs.filter(
      (job) => job.status === 'Running' || !seenJobIds.has(job.id)
    ).length;
  }
...
<div class="w-full">
  <button
    ...
    {#if unseenOrRunningCount > 0}
      <div
        class="absolute -top-1 -right-1 w-4 h-4 bg-secondary-300 rounded-full flex items-center justify-center text-[10px] text-white font-bold">
  ...
  {#if expanded}
    <div
      class="visible opacity-100 transition-all duration-200
      absolute left-20 bottom-4 z-50 w-96 h-96
      ...
      <div class="flex justify-between items-center px-3 py-2 border-b border-primary-500">
        ...
        <div class="flex items-center gap-2">
          ...
          {/if}
          <button onclick={toggleExpanded} class="text-gray-400 hover:text-white text-sm">
            &times;
          </button>
        </div>
      </div>
        ...
        {#each $jobs as job}
          {#if job.id === selectedJobId}
            <div class="flex-1 flex flex-col overflow-hidden p-3">
              <div class="flex justify-between items-center mb-2">
                <div class="flex items-center gap-2">
                  ...
                  <div class="flex items-center gap-1">
                    <span class="inline-block w-2 h-2 rounded-full {getStatusClass(job.status)}">
                    </span>
              ...
              <div
                class="flex-1 overflow-y-auto bg-primary-700 rounded p-2 font-mono text-xs whitespace-pre-wrap">
                {#if job.status === 'Running'}
                  <div class="text-yellow-400">Job is still running...</div>
                ...
                {:else}
                  <div class="text-gray-400">No output available</div>
                {/if}
              </div>
              ...
              <div class="mt-2 text-gray-300 text-xs">
                ...
                <div
                  class="font-mono text-xs bg-primary-700 rounded p-2 whitespace-pre-wrap mt-1 max-h-24 overflow-y-auto">
                  {job.command}
                </div>
              </div>
            </div>
            ...
            <button
              ...
              <div class="flex justify-between items-center">
                <div class="flex items-center">
                  <span class="inline-block w-2 h-2 mr-2 rounded-full {getStatusClass(job.status)}">
                  </span>
