...
<div class="h-full space-y-6 p-4">
  ...
  <Card padding="lg">
    <div class="flex items-center justify-between mb-6">
      ...
      <button class="px-4 sm:px-6 py-1.5 sm:py-2 bg-red-700 text-white rounded-full text-sm sm:text-base">Stop Daemon</button>
    </div>
    ...
    <div class="border-t border-gray-200 pt-6">
      ...
      {#if showVerbose}
        <div class="font-mono text-xs sm:text-sm bg-gray-900 text-gray-100 p-3 sm:p-4 rounded-lg space-y-1 overflow-x-auto">
          {#each logs as log}
            <div class={
      ...
      {:else}
        <div class="font-mono text-xs sm:text-sm space-y-2">
          {#each transactions as tx}
            <Card variant={tx.amount > 0 ? 'secondary' : 'primary'} padding="sm" className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-4">
              <span class="whitespace-nowrap">{tx.amount > 0 ? `+ ${tx.amount}` : '→ ' + Math.abs(tx.amount)} $VIRAL</span>
              <span class="text-gray-600 sm:text-inherit break-all sm:break-normal">{tx.description}</span>
            </Card>
          {/each}
        </div>
      {/if}
