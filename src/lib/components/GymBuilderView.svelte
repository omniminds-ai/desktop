<script lang="ts">
  ...
  async function handleGymModalSave(event: CustomEvent) {
    try {
      ...
      if (generatedResponse && generatedResponse.content && generatedResponse.content.apps && generatedResponse.content.apps.length > 0) {
  ...
  <div class="relative flex flex-col flex-wrap gap-1 border-b border-gray-200 mb-4">
    <div class="flex flex-row">
      <div class="flex items-center w-full gap-3">
        <h1 class="text-2xl font-bold text-gray-800 mr-3 truncate overflow-hidden text-ellipsis max-w-[calc(100vw-8em)]" title="{pool.name}">{pool.name}</h1>
        <!-- Power Button (Pause/Activate) with custom SVG -->
        <button
          class="p-1.5 cursor-pointer rounded-none transition-colors {pool.status === TrainingPoolStatus.live
            ? 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            : 'bg-green-500 text-white hover:bg-green-600'}"
          title="{pool.status === TrainingPoolStatus.live ? 'Pause Gym' : 'Activate Gym'}"
          on:click={() => onSave(pool, {
