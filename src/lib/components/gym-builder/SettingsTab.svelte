<script lang="ts">
  ...
  let limitType: 'per-task' | 'per-day' | 'total' = (pool.uploadLimit?.limitType || 'per-task') as
    | 'per-task'
    | 'per-day'
    | 'total';
...
<div class="space-y-6">
  <Card
    ...
    <div class="space-y-4">
      ...
      <div>
        <label for="enableUploadLimit" class="block text-sm font-medium mb-1">Upload Limits</label>
        <div class="space-y-3">
          <Checkbox
            ...
            <div class="pl-6 space-y-3">
              <div class="space-y-3">
                <div class="flex items-center gap-4">
                  <div class="flex-1">
                    <label for="select-limit-type" class="block text-sm font-medium mb-1">
                      Limit Type
                    </label>
                    <select
                      id="select-limit-type"
                  ...
                  <div class="flex-1">
                    <label for="limit-value" class="block text-sm font-medium mb-1">
                      Limit Value
                    </label>
                    <Input
                      id="limit-value"
                  ...
                  <div class="mt-2 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    {#if loadError}
                      <p class="text-xs mt-1 text-red-500">
                        Error loading stats. <button class="underline" onclick={fetchUploadStats}>
                          Retry
                        </button>
                      </p>
                    {:else}
                      <p class="text-xs mt-1 text-gray-500">
                        ...
                        {:else}
                          <button class="text-blue-500 underline" onclick={fetchUploadStats}>
                            Refresh stats
                          </button>
                        {/if}
                      </p>
                                    ...
                                    <td class="py-1.5">
                                      <div class="flex flex-col gap-1">
                                        <div class="flex justify-between text-xs">
                                          ...
                                          <span
                                            class="text-xs"
                                            style="color: {task.count >= limitValue * 0.9
                                              ? '#f44336'
                                              : task.count >= limitValue * 0.75
                                                ? '#ff9800'
                                                : '#4caf50'}">
                                          ...
                                          <div
                                            ...
                                            style="width: {Math.min(
                                              ...
                                              Math.round((task.count / limitValue) * 100)
                                                ...
                                                background-color: {task.count >= limitValue * 0.9
                                              ? '#f44336'
                                              : task.count >= limitValue * 0.75
                                                ? '#ff9800'
                                                : '#4caf50'};">
