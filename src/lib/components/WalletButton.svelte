<script lang="ts">
  ...
  onMount(async () => {
    if ($walletAddress) {
      omnisBalance = await getBalance($walletAddress);
  ...
  walletAddress.subscribe(async (address) => {
    if (address) {
      omnisBalance = await getBalance(address);
  ...
  <a
    ...
    <!-- Wallet Popup -->
        <div
          ...
          absolute top-0 right-full ml-1 w-64 p-4
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="font-semibold text-secondary-300">{formatNumber(omnisBalance)} $OMNIS</span>
            </div>
...
{:else}
  <a
    ...
    {#if variant !== 'large'}
      <div
        class="invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200 absolute top-0 right-full ml-2 w-40 p-3 bg-black text-gray-300 text-xs rounded-lg whitespace-nowrap text-center z-50 shadow-lg border border-gray-800">
