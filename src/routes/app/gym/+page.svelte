<script lang="ts">
  ...
  $: averageEarnings = recentSubmissions.length > 0 
    ? earnedThisMonth / recentSubmissions.length 
    : 0;
...
<div class="h-full max-w-7xl mx-auto overflow-x-hidden">
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    ...
    <Card padding="none" className="relative overflow-hidden border-0 shadow-md h-full">
      <div class="bg-gradient-to-br from-secondary-500 to-secondary-200 text-white h-full flex flex-col">
        <div class="p-4 flex-grow">
          ...
          <div class="text-lg font-semibold mt-1">{formatNumber(earnedThisMonth)} <span class="text-xs font-medium opacity-75">$VIRAL</span></div>
          ...
          <div class="text-base font-semibold mt-1">{formatNumber(averageEarnings)} <span class="text-xs font-medium opacity-75">$VIRAL</span></div>
        </div>
        
        <div class="flex border-t border-white/20 text-xs mt-auto">
          <a href="/app/gym/skills" class="flex-1 py-3 flex items-center justify-center gap-1.5 font-medium hover:bg-white/10 transition-colors">
            <Zap size={12} />
            <span>Skill Tree</span>
          </a>
          <div class="w-px bg-white/20"></div>
          <a href="/app/gym/history" class="flex-1 py-3 flex items-center justify-center gap-1.5 font-medium hover:bg-white/10 transition-colors">
            <History size={12} />
            <span>History</span>
          </a>
        </div>
        ...
        <div class="p-4 flex flex-col h-full">
          ...
          {#if pendingRecordings.length > 0}
              ...
              <div class="grid grid-cols-5 sm:grid-cols-6 gap-1.5">
                {#each pendingRecordings.slice(0, 8) as recording, i}
                  <a href="/app/gym/history" class="group">
                    <div class="aspect-square rounded-xl opacity-75 hover:opacity-100
                      {i % 4 === 0 ? 'bg-gradient-to-br from-pink-500/40 to-red-500/40' : 
                       i % 4 === 1 ? 'bg-gradient-to-br from-blue-500/40 to-cyan-500/40' : 
                       i % 4 === 2 ? 'bg-gradient-to-br from-green-500/40 to-emerald-500/40' : 
                      ...
                      {i % 4 === 0 ? 'hover:shadow-lg hover:shadow-pink-500/30' : 
                       i % 4 === 1 ? 'hover:shadow-lg hover:shadow-blue-500/30' : 
                       i % 4 === 2 ? 'hover:shadow-lg hover:shadow-green-500/30' : 
                                     'hover:shadow-lg hover:shadow-amber-500/30'} 
                      cursor-pointer">
                      <div class="relative">
                        <div class="w-10 h-10 rounded-full 
                          {i % 4 === 0 ? 'bg-gradient-to-br from-pink-400 to-red-500' : 
                           i % 4 === 1 ? 'bg-gradient-to-br from-blue-400 to-cyan-500' : 
                           i % 4 === 2 ? 'bg-gradient-to-br from-green-400 to-emerald-500' : 
                                         'bg-gradient-to-br from-amber-400 to-orange-500'} 
                          flex items-center justify-center mb-1 group-hover:animate-pulse overflow-hidden">
                          {#if recording.quest?.icon_url}
                            <img src={recording.quest.icon_url} alt={recording.quest.app || 'App'} class="w-6 h-6 object-contain" />
                          {:else}
                            <span class="text-white text-sm font-bold">{formatNumber(recording.quest?.reward?.max_reward || 0).split('.')[0]}</span>
                          {/if}
                        </div>
                      </div>
            ...
            <div class="mt-auto pt-4">
                <Button href="/app/gym/history" variant="primary" behavior="none" class="
