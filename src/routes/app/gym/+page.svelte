                        ...
                        <div
                          ...
                          {:else}
                            <span class="text-white text-sm font-bold">
                              {formatNumber(recording.quest?.reward?.max_reward || 0).split('.')[0]}
                            </span>
                          {/if}
                        </div>
                      </div>
  ...
  <div class="flex items-center justify-between mb-4 mt-6">
    <div class="flex items-center gap-2">
      ...
      {#if selectedCategories.size > 0}
        <div class="bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full text-xs font-medium">
          {selectedCategories.size} Categories
        </div>
      {/if}
      {#if minPrice > 1 || maxPrice < 99}
        <div class="bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full text-xs font-medium">
          {minPrice} - {maxPrice} $VIRAL
        </div>
      {/if}
    </div>
    ...
    <div class="flex items-center gap-2">
      {#if selectedCategories.size > 0 || minPrice > globalMinPrice || maxPrice < globalMaxPrice}
        <div class="flex items-center gap-1 text-xs text-gray-500">
          <button
            class="text-secondary-500 cursor-pointer hover:text-secondary-600 transition-colors hover:underline"
            onclick={() => {
      ...
      {/if}
      <button
        class="flex items-center gap-2 px-3 py-1.5 text-sm group cursor-pointer font-medium text-gray-700 hover:text-gray-900 transition-colors"
        onclick={() => (showFilters = !showFilters)}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="w-4 h-4 transition-transform group-hover:scale-125"
